...
public class ReportDController extends Controller {
    ...
    private void reportD(){ //00041923 method to create the D report with help of a button
        try { //00041923 tries the code below
            if (!facilitatorTextField.getText().equals("")) {
                ...
                PreparedStatement ps = db.prepareStatement("SELECT Client.clientId, Client.name, Card.cardNumber, SUM(Transaction.totalAmount) as totalAMOUNT, COUNT(Transaction.transactionId) as purchases" +
                        ...
                        " WHERE Card.facilitator = ? GROUP BY Client.clientId, Transaction.transactionId, Client.name, Card.cardNumber, Client.name, Card.cardNumber"); //00041923 query to execute where select a client, his card, how many transactions did and the total spent
