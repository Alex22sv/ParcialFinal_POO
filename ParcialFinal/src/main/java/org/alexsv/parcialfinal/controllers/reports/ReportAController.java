...
public class ReportAController {
    ...
    @FXML
    private DatePicker firstDate; //00041923 first date to compare
    @FXML
    private DatePicker secondDate; //0004923 second date to compare
    @FXML
    protected void reportA() throws SQLException, ParseException, IOException { //00049123 method to create the A report with help of a button
        ...
        ps.setDate(1, Date.valueOf(firstDate.getValue())); //00041923 put the parameters from the prepareStatement in first index, in this case a date
        ps.setDate(2,Date.valueOf(secondDate.getValue())); //00041923 put the parameters from the prepareStatement in second index, in this case a date
